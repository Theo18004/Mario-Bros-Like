/**
 * @file map.c
 * @brief Chargement des données CSV et rendu des tuiles.
 */
#include "map.h"
#include <stdio.h>
#include <stdlib.h>
...
void draw_parallax_bg(SDL_Renderer* renderer, SDL_Texture* tex, int scrollX, int scrollY, float speedX, float speedY, int screenW, int screenH, int offsetY) {
    ...
    SDL_Rect dest = { posX, posY, finalW, th * MAP_SCALE };
    SDL_RenderCopy(renderer, tex, NULL, &dest);
    if (dest.x + finalW < screenW) { dest.x += finalW; SDL_RenderCopy(renderer, tex, NULL, &dest); }
}